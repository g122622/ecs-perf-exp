...
#include "hybrid/include/world.hpp"
...
int main(int argc, char* argv[]) {
    std::cout << "====================================" << std::endl;
    std::cout << "  Hybrid Benchmark - ECS vs OOP Performance" << std::endl;
    std::cout << "====================================" << std::endl;
    std::cout << std::endl;
    ...
    for (const auto& config : configs) {
        std::cout << "\n--- Running Hybrid Benchmark: " << config.entityCount
        ...
        // Create world
        HybridWorld world(config.entityCount);
        ...
        for (size_t frame = 0; frame < config.frameCount; ++frame) {
            ...
            data.updateTimeMs = data.frameTimeMs;  // Hybrid doesn't measure separately
    ...
    runner.writeFrameTimeCSV((outputPath / "hybrid_frame_time.csv").string(), allFrameData, scales);
    runner.writeMemoryCSV((outputPath / "hybrid_memory.csv").string(), allMemoryStats);
    runner.writeCreationCSV((outputPath / "hybrid_creation.csv").string(), allCreationStats);
    runner.writeHWCountersCSV((outputPath / "hybrid_hw_counters.csv").string(), allHWCounters);

    std::cout << "\n====================================" << std::endl;
    std::cout << "  Hybrid Benchmark Complete!" << std::endl;
    std::cout << "====================================" << std::endl;
