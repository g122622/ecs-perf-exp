...
int main(int argc, char* argv[]) {
    std::cout << "====================================" << std::endl;
    std::cout << "  ECS Benchmark - ECS vs OOP Performance" << std::endl;
    std::cout << "====================================" << std::endl;
    std::cout << std::endl;
    ...
    for (const auto& config : configs) {
        std::cout << "\n--- Running ECS Benchmark: " << config.entityCount
        ...
        world.setSeed(config.randomSeed);
        ...
        for (size_t frame = 0; frame < config.frameCount; ++frame) {
            ...
            data.updateTimeMs = data.frameTimeMs;  // ECS没有单独测量
    ...
    runner.writeFrameTimeCSV(outputPath + "/ecs_frame_time.csv", allFrameData, scales);
    runner.writeMemoryCSV(outputPath + "/ecs_memory.csv", allMemoryStats);
    runner.writeCreationCSV(outputPath + "/ecs_creation.csv", allCreationStats);
    runner.writeHWCountersCSV(outputPath + "/ecs_hw_counters.csv", allHWCounters);

    std::cout << "\n====================================" << std::endl;
    std::cout << "  ECS Benchmark Complete!" << std::endl;
    std::cout << "====================================" << std::endl;
